...
/**
 * KingbaseES Source（Reader）适配器。
 */
public class KingbaseSourceAdapter implements SourceDatabaseAdapter {
    ...
    public CapabilityResult capability(Connection connection) {
        try {
            ...
            return CapabilityResult.blocked("DATABASE_CONNECTION_FAILED",
                    "KingbaseES Source 元数据读取失败", List.of("检查连接串、账号 SELECT 权限与数据库服务状态"));
        }
    }
}
