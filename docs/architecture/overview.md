...
源数据库（KingbaseES / openGauss）
   ...
   └─────────────┘ ◀──── Receipt / 409 ──── └─────────────┘
                                                  │ UPSERT / UPSERT_NO_OVERWRITE / INSERT_ONLY / REPLACE_ALL
                                                  ▼
...
| Agent | 控制台向远程 Sink 端下发数据源配置、连接测试与目标元数据探查的通道 |
...
- 所有部署形态统一使用 `/mic-data-sync/` URL 前缀（`MIC_SYNC_CONTEXT_PATH`），开发、测试与生产一致。
